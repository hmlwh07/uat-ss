...
import { DISPLAY_COL, ELEMENT_COL } from './marketing-sales.others.const';

@Component({
  selector: 'app-marketing-sales-others',
  templateUrl: './marketing-sales-others.component.html',
  styleUrls: ['./marketing-sales-others.component.scss']
})
export class MarketingSalesOthersComponent implements OnInit {
  ...
  displayInputDialog(data?) {
    ...
    this.fnaBRAMInputService.displayInput(this.fnaId, this.data, this.menuType, 'Other').then(result => {
      if (result) {
        this.getAll();
      }
    })
  ...
  async getAll() {
    await this.marketingSalesService.getAllMarketingSalesByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, MARKETING_SALES.OTHERS.toString());
