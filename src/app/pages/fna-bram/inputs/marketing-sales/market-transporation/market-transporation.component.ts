...
import { INBOUND_LOGISTICS, MARKETING_SALES } from '../../input-data-dialog/input.data.enum';
import { InputsService } from '../../inputs.manage.service';
import { MarketingSalesService } from '../marketing-sales.manage.service';
import { DISPLAY_COL, ELEMENT_COL } from './market-transporation.const';

@Component({
  selector: 'app-market-transporation',
  templateUrl: './market-transporation.component.html',
  styleUrls: ['./market-transporation.component.scss']
})
export class MarketTransporationComponent implements OnInit {
  ...
  async getAll() {
    await this.marketingSalesService.getAllMarketingSalesByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, MARKETING_SALES.TRANSPORATION.toString());
