...
import { DISPLAY_COL, ELEMENT_COL } from './service-infrastructure.const';

@Component({
  selector: 'app-service-infrastructure',
  templateUrl: './service-infrastructure.component.html',
  styleUrls: ['./service-infrastructure.component.scss']
})
export class ServiceInfrastructureComponent implements OnInit {
  ...
  constructor(private cdf: ChangeDetectorRef, private fnaBRAMInputService: FNABRAMInputService,
    private bramServiceService: BRAMServiceService, private alertService: AlertService,
  ...
  delete(id) {
    this.alertService.activate('Are you sure you want to delete?', 'Warning Message').then(result => {
      if (result) {
        this.bramServiceService.deleteBRAMServiceById(id).toPromise().then(async (res: any) => {
          if (res) {
            this.getAll();
  ...
  async getAll() {
    await this.bramServiceService.getAllBRAMServiceByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, SERVICE.SERVICE_INFRASTRUCTURER.toString());
