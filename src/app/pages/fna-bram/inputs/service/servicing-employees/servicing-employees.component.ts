...
@Component({
  ...
  templateUrl: './servicing-employees.component.html',
  styleUrls: ['./servicing-employees.component.scss']
})
export class ServicingEmployeesComponent implements OnInit {
  ...
  displayInputDialog(data?) {
    ...
    this.fnaBRAMInputService.displayInput(this.fnaId, this.data, this.menuType, 'Servicing employees').then(result => {
      if (result) {
        this.getAll();
      }
    })
  ...
  async getAll() {
    await this.bramServiceService.getAllBRAMServiceByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, SERVICE.SERVICING_EMPLOYEES.toString());
