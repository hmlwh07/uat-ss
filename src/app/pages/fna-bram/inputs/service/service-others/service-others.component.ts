...
import { DISPLAY_COL, ELEMENT_COL } from './service-others.const';

@Component({
  selector: 'app-service-others',
  templateUrl: './service-others.component.html',
  styleUrls: ['./service-others.component.scss']
})
export class ServiceOthersComponent implements OnInit {
  ...
  async getAll() {
    await this.bramServiceService.getAllBRAMServiceByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, SERVICE.OTHERS.toString());
