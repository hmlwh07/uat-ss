...
import { DISPLAY_COL, ELEMENT_COL } from './good.in.transporation.const';

@Component({
  selector: 'app-good-in-transportation',
  templateUrl: './good-in-transportation.component.html',
  styleUrls: ['./good-in-transportation.component.scss']
})
export class GoodInTransportationComponent implements OnInit {
  ...
  displayInputDialog(data?) {
    ...
    this.fnaBRAMInputService.displayInput(this.fnaId, this.data, this.menuType, 'goods_in_transportation').then(result => {
      if (result) {
        this.getAll();
      }
    })
  ...
  async getAll() {
    await this.outboundLogisticsService.getAllOutboundLogisticsByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, OUTBOUND_LOGISTICS.GOODS_TRANSPORTATION.toString());
