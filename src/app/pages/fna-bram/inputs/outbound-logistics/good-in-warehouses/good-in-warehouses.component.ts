...
import { DISPLAY_COL, ELEMENT_COL } from './good.in.warehouses.const';

@Component({
  selector: 'app-good-in-warehouses',
  templateUrl: './good-in-warehouses.component.html',
  styleUrls: ['./good-in-warehouses.component.scss']
})
export class GoodInWarehousesComponent implements OnInit {
  ...
  displayInputDialog(data?) {
    ...
    this.fnaBRAMInputService.displayInput(this.fnaId, this.data, this.menuType, 'Goods In Warehouses').then(result => {
      if (result) {
        this.getAll();
      }
    })
  ...
  async getAll() {
    await this.outboundLogisticsService.getAllOutboundLogisticsByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, OUTBOUND_LOGISTICS.GOODS_WAREHOUSE.toString());
