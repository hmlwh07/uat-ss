...
import { DISPLAY_COL, ELEMENT_COL } from './outbound-logistics-others.const';

@Component({
  selector: 'app-outbound-logistics-others',
  templateUrl: './outbound-logistics-others.component.html',
  styleUrls: ['./outbound-logistics-others.component.scss']
})
export class OutboundLogisticOthersComponent implements OnInit {
  ...
  async getAll() {
    await this.outboundLogisticsService.getAllOutboundLogisticsByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      console.log('getAllOutboundLogisticsByfnaId', res);

      this.dataList = this.inputsService.getAllInputByType(res, OUTBOUND_LOGISTICS.OTHERS.toString());
