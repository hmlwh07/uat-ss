...
import { DISPLAY_COL, ELEMENT_COL } from './factory-building.const';

@Component({
  selector: 'app-factory-building',
  templateUrl: './factory-building.component.html',
  styleUrls: ['./factory-building.component.scss']
})
export class FactoryBuildingComponent implements OnInit {
  ...
  async getAll() {
    await this.operationService.getAllOperationByFnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, OPERATION.FACTORY_BUILDING.toString());
