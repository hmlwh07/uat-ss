...
import { DISPLAY_COL, ELEMENT_COL } from './technology-others.const';

@Component({
  selector: 'app-technology-others',
  templateUrl: './technology-others.component.html',
  styleUrls: ['./technology-others.component.scss']
})
export class TechnologyOthersComponent implements OnInit {
  ...
  async getAll() {
    await this.technologyService.getAllTechnologyByfnaId(this.fnaId).toPromise().then(async (res: any) => {
      this.dataList = this.inputsService.getAllInputByType(res, TECHNOLOGY.OTHER.toString());
