...
const API_MENU_URL = `${environment.apiUrl}/menu/role`
...
export class MenuDataRoleService extends BizOperationService<any, number>{

  menuData = new BehaviorSubject<any[]>([])
  menuRoleData = new BehaviorSubject<any[]>([])
  ...
  getMenusRoleData(roleId) {
    ...
    return this.findAllWithRoleQuery("/"+roleId+"?"+"lan="+language).pipe(map((menus) => {
